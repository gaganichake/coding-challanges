 ...
 * <p>
 * The position within the wall is given from the top-left. This position may fall anywhere within a brick
 * (not necessarily at the start) and identifies the brick where it points to. Additionally, depending on
 * the brick width, there can be multiple positions to identify the same brick.
 * <p>
...
public class Wall {
    ...
    private static int analyze_brick(int[][] wall, int i, int j){
        ...
        int brickValue = wall[i][j];
        int neighbors = 0;

        // Check all four directions (up, down, left, right)
        if (i > 0 && wall[i - 1][j] == brickValue) neighbors++; // Up
        if (i < height - 1 && wall[i + 1][j] == brickValue) neighbors++; // Down
        if (j > 0 && wall[i][j - 1] == brickValue) neighbors++; // Left
        if (j < width - 1 && wall[i][j + 1] == brickValue) neighbors++; // Right
