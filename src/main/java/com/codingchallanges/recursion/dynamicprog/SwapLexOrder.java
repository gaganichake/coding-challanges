...
public class SwapLexOrder {
    ...
    static String solution(String str, int[][] pairs) {
        ...
        List<Set<Integer>> listOfGraph = findConnectedInices(pairs);
//        System.out.println(listOfGraph);
        ...
        for(Set<Integer> group : listOfGraph) {
            ...
            List<Character> chars = group.stream().map(integer -> str.charAt(integer.intValue()-1)).sorted(Comparator.reverseOrder()).collect(Collectors.toList());
//            System.out.println("chars = " + chars);
    ...
    private static List<Set<Integer>> findConnectedInices(int[][]  pairs) {
